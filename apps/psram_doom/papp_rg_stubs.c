/*
 * Retro-go API stubs for PSRAM Doom app.
 *
 * The prboom engine (compiled with -DRETRO_GO) references various rg_*
 * functions. We provide minimal stubs that route through app_services_t
 * or are no-ops where the feature is not needed.
 */
#include "psram_app.h"
#include <rg_system.h>
#include <string.h>
...
/* Surface used by I_FinishUpdate (created in papp_i_video.c) */
extern rg_surface_t *papp_get_surface(void);

/* ── rg_system ───────────────────────────────────────────────────────── */

static rg_app_t s_app = { .romPath = "/sd/roms/doom/doom.wad" };
...
rg_surface_t *rg_surface_create(int width, int height, int format, uint32_t alloc_flags)
{
    ...
    if (!s->data) {
        s->data = _papp_svc->mem_alloc(width * height);
    }
    if (s->data)
        __builtin_memset(s->data, 0, width * height);
    /* Allocate palette for paletted formats */
...
{
    /* The actual rendering is done by I_FinishUpdate in papp_i_video.c.
     * This stub is called from event handlers but we can just no-op. */
    (void)update;
    (void)flags;
}
...
uint32_t rg_input_read_gamepad(void)
{
    ...
    uint32_t mask = 0;
    /* Map papp_gamepad_state array to retro-go bitmask */
...
{
    if (_papp_svc)
        _papp_svc->log_printf("DOOM GUI: %s — %s\n", title ? title : "", msg ? msg : "");
}
