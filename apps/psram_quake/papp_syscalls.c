/*
 * Newlib syscall stubs + heap wrappers for Quake PSRAM App.
 *
...
{
    if (_papp_svc) {
        _papp_svc->log_printf("QUAKE cleanup: freeing %d tracked allocations\n", alloc_count);
...
/* heap_caps_malloc — used by engine for specific memory allocations.
 * Internal RAM is scarce — redirect large INTERNAL requests to PSRAM. */
void *heap_caps_malloc(size_t size, uint32_t caps)
{
    /* For a PSRAM app, internal RAM is precious. Route big allocs to PSRAM. */
    if (size >= 1024)
        caps = PAPP_MEM_CAP_SPIRAM;
