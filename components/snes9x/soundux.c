...
bool S9xInitSound(int32_t buffer_ms, int32_t lag_ms)
{
   ...
   if (!wave) { printf("S9xInitSound: wave alloc failed\n"); return false; }
   MixBuffer = malloc(sample_count * sizeof(MixBuffer[0]));
   if (!MixBuffer) { printf("S9xInitSound: MixBuffer alloc failed\n"); return false; }
   EchoBuffer = malloc(sample_count * sizeof(EchoBuffer[0]));
   if (!EchoBuffer) { printf("S9xInitSound: EchoBuffer alloc failed\n"); return false; }
   /* Echo (96KB) may not fit in DRAM when SRAM is also in DRAM — use PSRAM */
   Echo = heap_caps_malloc(24000 * sizeof(Echo[0]), MALLOC_CAP_SPIRAM);
   if (!Echo) { printf("S9xInitSound: Echo alloc failed (PSRAM)\n"); return false; }
   so.playback_rate = 0;
   S9xResetSound(true);
