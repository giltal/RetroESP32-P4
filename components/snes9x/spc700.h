...
/* Raw SPC700 single-instruction step — always available */
#define APU_RUN_ONE() \
{ \
...
#ifdef APU_SYNC_SCANLINE
/*
 * Scanline-accurate APU: skip per-instruction sync in the main CPU
 * loop.  Instead, batch-execute the SPC700 once per HBlank (scanline).
 * This removes ~30,000+ branch checks per frame.
 */
#define APU_EXECUTE1() do {} while(0)
#define APU_EXECUTE()  do {} while(0)

/* Called from S9xDoHBlankProcessing to catch up the APU */
#define APU_SYNC_BATCH() \
    if (IAPU.APUExecuting) \
        while (APU.Cycles <= CPU.Cycles) \
            APU_RUN_ONE()
#else
/* Original per-instruction sync */
#define APU_EXECUTE1() APU_RUN_ONE()
...
#define APU_SYNC_BATCH() do {} while(0)
#endif
