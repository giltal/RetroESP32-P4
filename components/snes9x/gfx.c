...
/* Pristine Z-buffer pointers for corruption repair.
 * Set once by S9xInitDisplay (snes_run.c), checked every frame. */
uint8_t *saved_ZBuffer    = NULL;
uint8_t *saved_SubZBuffer = NULL;

void S9xStartScreenRefresh(void)
{
   /* Guard: repair GFX Z-buffer pointers if something corrupted them.
    * DKC triggers mode-5 colour math that has historically caused a
    * Load-access-fault because GFX.SubZBuffer was found zeroed/trashed. */
   if (saved_SubZBuffer && GFX.SubZBuffer != saved_SubZBuffer) {
      GFX.SubZBuffer = saved_SubZBuffer;
   }
   if (saved_ZBuffer && GFX.ZBuffer != saved_ZBuffer) {
      GFX.ZBuffer = saved_ZBuffer;
   }
   ...
   if (IPPU.RenderThisFrame)
   {
      ...
      /* Force interlace off — our display is fixed 256-wide; interlaced
       * rendering doubles PPL to 512 which overruns the ZBuffer (sized
       * for 256).  DKC triggers this and crashes. */
...
void S9xUpdateScreen(void)
{
   ...
   /* Repair corrupted Z-buffer pointers before any rendering */
   if (saved_SubZBuffer && GFX.SubZBuffer != saved_SubZBuffer)
      GFX.SubZBuffer = saved_SubZBuffer;
   if (saved_ZBuffer && GFX.ZBuffer != saved_ZBuffer)
      GFX.ZBuffer = saved_ZBuffer;
