...
{
   /* DEBUG: track writes to $3000+ range */
   static uint32_t ppu_call_count = 0;
   static uint16_t ppu_max_addr = 0;
   static uint32_t sfx_range_count = 0;
   static uint32_t else_branch_count = 0;
   static uint32_t sfx_ctrl_count = 0; /* $3000-$303F */
   ppu_call_count++;
   if (Address > ppu_max_addr) ppu_max_addr = Address;
   if (Address >= 0x3000) {
      sfx_range_count++;
      if (Address < 0x3040) sfx_ctrl_count++;
   }
   if (Address <= 0x2183)
   {
      switch (Address)
   ...
   else
   {
      ...
      else if (Address >= 0x3000 && Address < 0x3400) {
         /* Write to FillRAM for cache/data area ($3040+),
          * control registers ($3000-$303F) handled by S9xSetSuperFX */
         if (Address >= 0x3040)
            Memory.FillRAM[Address] = Byte;
         if (Settings.SuperFX)
...
IRAM_ATTR uint8_t S9xGetPPU(uint16_t Address)
{
   ...
   else
   {
      ...
      if (Address == 0x3030)
      {
         /* Return actual GSU SFR low byte (flags + GO) */
         byte = (uint8_t)(GSU.vStatusReg & 0xFF);
         CPU.WaitAddress = CPU.PCAtOpcodeStart;
         static int sfr_read_count = 0;
         if (sfr_read_count < 5) {
            uint16_t cpu_pc = (uint16_t)(CPU.PCAtOpcodeStart - CPU.PCBase);
            ESP_LOGI("SFX", "RD SFR=%02X CPU_PC=%02X:%04X SP=%04X",
                byte, ICPU.Registers.PB, cpu_pc, ICPU.Registers.S.W);
            sfr_read_count++;
         }
      }
      else if (Address == 0x3031)
      {
         /* Return actual GSU SFR high byte */
         byte = (uint8_t)((GSU.vStatusReg >> 8) & 0xFF);
         CLEAR_IRQ_SOURCE(GSU_IRQ_SOURCE);
         byte &= 0x7f; /* Clear IRQ flag on read */
         Memory.FillRAM[0x3031] = byte;
      }
      else if (Address == 0x303B)
      {
         byte = 0x04; /* VCR = SuperFX2 version code */
      }
      return byte;
   }
   return byte;
