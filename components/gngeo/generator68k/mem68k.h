...
/* Compatibility: map memory.cpu / memory.bios to actual struct paths */
#include "memory.h"
#include "roms.h"
#define COMPAT_MEM_CPU  (memory.rom.cpu_m68k.p)
#define COMPAT_MEM_BIOS (memory.rom.bios_m68k.p)
...
static __inline__ uint8 fetchbyte(uint32 addr) {
	...
	addr&=0xfffff;
	return (*(uint8 *) (COMPAT_MEM_CPU + bankaddress + addr));
    }
    if (adup >=0x000 && adup <=0x0ff) { /* cpu bank 0 */
	addr&=0xfffff;
	return (*(uint8 *) (COMPAT_MEM_CPU + addr));
    }
    if (adup >=0xc00 && adup <=0xc1f) { /* bios */
	addr&=0x1ffff;
	return (*(uint8 *) (COMPAT_MEM_BIOS + addr));
    }
...
static __inline__ uint16 fetchword(uint32 addr) {
    ...
    if (adup >=0x200 && adup <=0x2ff) { /* banked cpu */
	addr&=0xfffff;
	return LOCENDIAN16(*(uint16 *) (COMPAT_MEM_CPU + bankaddress + addr));
    }
    if (adup >=0x000 && adup <=0x0ff) { /* cpu bank 0 */
	addr&=0xfffff;
	return LOCENDIAN16(*(uint16 *) (COMPAT_MEM_CPU + addr));
    }
    if (adup >=0xc00 && adup <=0xc1f) { /* bios */
	addr&=0x1ffff;
	return LOCENDIAN16(*(uint16 *) (COMPAT_MEM_BIOS + addr));
    }
...
static __inline__ uint32 fetchlong(uint32 addr) {
    ...
    if (adup >=0x200 && adup <=0x2ff) { /* banked cpu */
	addr&=0xfffff;
	return (LOCENDIAN16(*(uint16 *) (COMPAT_MEM_CPU + bankaddress + addr))<< 16) |
	    LOCENDIAN16(*(uint16 *) (COMPAT_MEM_CPU + bankaddress + addr + 2 ));
    }
    if (adup >=0x000 && adup <=0x0ff) { /* cpu bank 0 */
	addr&=0xfffff;
	return (LOCENDIAN16(*(uint16 *) (COMPAT_MEM_CPU + addr))<< 16) |
	    LOCENDIAN16(*(uint16 *) (COMPAT_MEM_CPU + addr + 2));
    }
    if (adup >=0xc00 && adup <=0xc1f) { /* bios */
	addr&=0x1ffff;
	return (LOCENDIAN16(*(uint16 *) (COMPAT_MEM_BIOS + addr))<< 16) |
	    LOCENDIAN16(*(uint16 *) (COMPAT_MEM_BIOS + addr + 2));
    }
#else
    if (adup >=0x100 && adup <=0x10F) { /* RAM */
	addr&=0xffff;
    ...
    if (adup >=0x200 && adup <=0x2ff) { /* banked cpu */
	addr&=0xfffff;
	return LOCENDIAN32(*(uint32 *) (COMPAT_MEM_CPU + bankaddress + addr));
    }
    if (adup >=0x000 && adup <=0x0ff) { /* cpu bank 0 */
	addr&=0xfffff;
	return LOCENDIAN32(*(uint32 *) (COMPAT_MEM_CPU + addr));
    }
    if (adup >=0xc00 && adup <=0xc1f) { /* bios */
	addr&=0x1ffff;
	return LOCENDIAN32(*(uint32 *) (COMPAT_MEM_BIOS + addr));
    }
#endif
