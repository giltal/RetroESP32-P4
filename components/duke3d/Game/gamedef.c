...
void loadefs(char  *filenam, char  *mptr, int readfromGRP)
{
    ...
    *script = (int32_t) scriptptr;
    ...
    else
    {
		...
		if(readfromGRP == 0 && conVersion != 13 && (getGRPcrc32(0)==CRC_BASE_GRP_SHAREWARE_13 ||
			getGRPcrc32(0)==CRC_BASE_GRP_FULL_13) && !getGRPcrc32(1))
		{
			printf("\nCON/GRP version mismatch (v1.3 GRP + v1.4/1.5 CON). Auto-reloading internal CON files.\n");
#ifdef PLATFORM_ESP32
			kbdKey = 'y';
#else
			printf("Reload normal GRP internal *.CON files? (Y/N) : ");
			do
				kbdKey = getch() | ' ';
		...
		else if(readfromGRP == 0 && conVersion != 15 && getGRPcrc32(0)==CRC_BASE_GRP_ATOMIC_15 && !getGRPcrc32(1))
		{
			printf("\nCON/GRP version mismatch (v1.5 ATOMIC GRP + v1.3/1.4 CON). Auto-reloading internal CON files.\n");
#ifdef PLATFORM_ESP32
			kbdKey = 'y';
#else
			printf("Reload normal GRP internal *.CON files? (Y/N) : ");
		...
		}else if(readfromGRP == 0 && conVersion != 14 && getGRPcrc32(0)==CRC_BASE_GRP_PLUTONIUM_14 && !getGRPcrc32(1))
		{
			printf("\nCON/GRP version mismatch (v1.4 PLUTONIUM GRP + v1.3/1.5 CON). Auto-reloading internal CON files.\n");
#ifdef PLATFORM_ESP32
			kbdKey = 'y';
#else
			printf("Reload normal GRP internal *.CON files? (Y/N) : ");
