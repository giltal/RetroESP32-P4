...
{
    /* ESP32-P4 PSRAM app: skip heavy engine/audio cleanup — the papp_main
       cleanup sequence handles everything (kill tasks, sound, fds, heap).
       The original cleanup (SoundShutdown, uninitengine, SDL_DestroyMutex
       etc.) can crash on embedded due to shared audio state and freed
       resources being accessed during teardown. */
    if(msg && *msg && *msg != ' ')
        printf("gameexit: %s\n", msg);
...
void Logo(void)
{
	...
	/* Skip cinematics on ESP32-P4 to speed up startup */
	ud.showcinematics = 0;
...
void Startup(void)
{
   ...
   printf("[DUKE3D-DIAG] CONFIG_ReadSetup START\n");
   CONFIG_ReadSetup();
   printf("[DUKE3D-DIAG] CONFIG_ReadSetup DONE\n");

   printf("[DUKE3D-DIAG] compilecons START\n");
   compilecons();
   printf("[DUKE3D-DIAG] compilecons DONE\n");
...
//}
   ...
   printf("[DUKE3D-DIAG] CONTROL_Startup START\n");
   CONTROL_Startup( ControllerType, &GetTime, TICRATE );
   printf("[DUKE3D-DIAG] CONTROL_Startup DONE\n");

// CTW - MODIFICATION
// initengine(ScreenMode,ScreenWidth,ScreenHeight);
   printf("[DUKE3D-DIAG] initengine START\n");
   initengine();
   printf("[DUKE3D-DIAG] initengine DONE\n");
// CTW END - MODIFICATION
   inittimer(TICRATE);

   printf("[DUKE3D-DIAG] loadpics START\n");
   puts("Loading art header.");

  loadpics("tiles000.art", "\0");
   printf("[DUKE3D-DIAG] loadpics DONE\n");
...
void findGRPToUse(char * groupfilefullpath){
    ...
    /* No opendir/readdir in papp — use fixed GRP path */
    sprintf(groupfilefullpath, "%s/duke3d.grp", getGameDir());
}
#else
...
int main(int argc,char  **argv)
{
	...
	printf("[DIAG] main() entered\n"); fflush(stdout);
	printf("*** Chocolate DukeNukem3D v%d.%d ***\n\n", CHOCOLATE_DUKE_REV_X, CHOCOLATE_DUKE_REV_DOT_Y);
    fflush(stdout);

    /* ESP32-P4: force single-player globals. The .data initializer
       (numplayers=1 in dummy_multi.c) may be lost in PSRAM XIP binary. */
    numplayers = 1;
    myconnectindex = 0;
    ...
    printf("[DUKE3D-DIAG] calling Startup()\n");
    Startup();
    printf("[DUKE3D-DIAG] Startup() returned\n");
   ...
   puts("Loading palette/lookups.");
    ...
    {
        { static int _gl_cnt = 0;
          if ((_gl_cnt & 255) == 0) { printf("[GL] i=%d gm=0x%x tc=%d\n", _gl_cnt, (unsigned)ps[myconnectindex].gm, (int)totalclock); fflush(stdout); }
          _gl_cnt++;
        }
    	sampletimer();
...
int32_t playback(void)
{
	...
	*/
	...
	if(numplayers < 2 && ud.multimode_bot<2) foundemo = 0; /* Skip demo on ESP32-P4 — go straight to menu */
    ...
    if(foundemo == 0)
    {
        ...
        printf("[DIAG] playback: palto fade-in\n"); fflush(stdout);
        for(t=0;t<63;t+=7) palto(0,0,0,t);
        printf("[DIAG] playback: drawbackground\n"); fflush(stdout);
        drawbackground();
        printf("[DIAG] playback: menus\n"); fflush(stdout);
        ...
        CONSOLE_Render();
        ps[myconnectindex].palette = palette;
        printf("[DIAG] playback: nextpage\n"); fflush(stdout);
        nextpage();
        printf("[DIAG] playback: palto fade-out\n"); fflush(stdout);
        for(t=63;t>0;t-=7) 
		{
			palto(0,0,0,t);
		}
    ...
    printf("[DIAG] playback: entering menu loop (foundemo=%d reccnt=%d)\n", foundemo, (int)ud.reccnt); fflush(stdout);
    { static int _ml_cnt = 0; _ml_cnt = 0; (void)_ml_cnt; }
    while (ud.reccnt > 0 || foundemo == 0)
    {
        { static int _ml_cnt = 0;
          if ((_ml_cnt & 31) == 0) { printf("[DIAG] menu loop iter=%d gm=0x%x tc=%d\n", _ml_cnt, (unsigned)ps[myconnectindex].gm, (int)totalclock); fflush(stdout); }
          _ml_cnt++;
        }
