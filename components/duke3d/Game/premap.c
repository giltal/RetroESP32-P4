...
void precachenecessarysounds(void)
{
    ...
    if (FXDevice == NumSoundCards) { printf("[DIAG] precache sounds: FXDevice==NumSoundCards, skip\n"); fflush(stdout); return; }
    j = 0;
    ...
    for(i=0;i<NUM_SOUNDS;i++)
        if(Sound[i].ptr == 0)
        {
            ...
            if( (j&7) == 0 )
                getpackets();
            if( (j&31) == 1 ) {
                printf("[DIAG] snd j=%d i=%d\n", j, i);
                fflush(stdout);
            }
            getsound(i);
        }
    printf("[DIAG] precachenecessarysounds DONE (%d sounds processed)\n", j);
    fflush(stdout);
}
...
void cacheit(void)
{
    ...
    /* Skip sound precaching — on ESP32-P4, the hundreds of SD card
       seeks during precachenecessarysounds cause the system to hang.
       Sounds load on-demand via sounds.c:sound() when first played. */
    printf("[DIAG] cacheit: skipping sound precache\n"); fflush(stdout);

    printf("[DIAG] cachegoodsprites START\n"); fflush(stdout);
    cachegoodsprites();
    printf("[DIAG] cachegoodsprites DONE\n"); fflush(stdout);
    ...
    printf("[DIAG] wall tloadtile DONE (numwalls=%d)\n", numwalls);
...
void docacheit(void)
{
    ...
    printf("[DIAG] docacheit DONE (%d tiles loaded)\n", (int)j); fflush(stdout);
...
void newgame(uint8_t  vn,uint8_t  ln,uint8_t  sk)
{
    ...
    if(globalskillsound >= 0) {
        int _snd_wait = 0;
        while(Sound[globalskillsound].lock>=200 && _snd_wait < 3000) { _snd_wait++; }
    }
    globalskillsound = -1;
...
if (!VOLUMEONE)
{
    ...
    printf("[DIAG] enterlevel: cacheit START\n"); fflush(stdout);
    cacheit();
    printf("[DIAG] enterlevel: docacheit START\n"); fflush(stdout);
    docacheit();
    printf("[DIAG] enterlevel: caching DONE\n"); fflush(stdout);

    if(ud.recstat != 2)
    {
        printf("[DIAG] enterlevel: playmusic\n"); fflush(stdout);
        music_select = (ud.volume_number*11) + ud.level_number;
        playmusic(&music_fn[0][music_select][0]);
        printf("[DIAG] enterlevel: playmusic DONE\n"); fflush(stdout);
    }
...
//    if(ud.level_number == 0 && ud.recstat != 2) FTA(40,&ps[myconnectindex]);
    ...
    for(i=connecthead;i>=0;i=connectpoint2[i])
     ...
     printf("[DIAG] enterlevel: resetmys\n"); fflush(stdout);
     resetmys();
     ...
     flushpackets();
     printf("[DIAG] enterlevel: waitforeverybody\n"); fflush(stdout);
     waitforeverybody();
     printf("[DIAG] enterlevel: waitforeverybody DONE\n"); fflush(stdout);
     ...
     printf("[DIAG] enterlevel: resettimevars\n"); fflush(stdout);
     resettimevars();  // Here we go
     printf("[DIAG] enterlevel: DONE - entering game loop\n"); fflush(stdout);
