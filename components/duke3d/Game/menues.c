...
int loadpheader(uint8_t  spot,int32 *vn,int32 *ln,int32 *psk,int32 *nump)
{
    ...
    int32_t bv;
     ...
     printf("[LOADHDR] TCkopen4load('%s', 0)\n", fn);
     if ((fil = TCkopen4load(fn,0)) == -1) {
         printf("[LOADHDR] file not found\n");
         return(-1);
     }
     printf("[LOADHDR] file opened OK, fil=%d\n", fil);

     tiles[MAXTILES-3].lock = 255;

     kdfread(&bv,4,1,fil);
     printf("[LOADHDR] bv=%d (expected %d)\n", bv, BYTEVERSION);
     if(bv != BYTEVERSION)
     {
        printf("[LOADHDR] VERSION MISMATCH\n");
...
int loadplayer(int8_t spot)
{
     ...
     kdfread(&bv,4,1,fil);
     /* ESP32-P4: force numplayers=1 for version check log */
     numplayers = 1;
     printf("[LOAD] bv=%d (expected %d), numplayers=%d\n", bv, BYTEVERSION, numplayers);
     if(bv != BYTEVERSION)
     {
        FTA(114,&ps[myconnectindex],1);
        kclose(fil);
     ...
     /* ESP32-P4: always single player — force numplayers=1.
        The .data initializer from dummy_multi.c may not survive PSRAM
        XIP loading, leaving numplayers=0 in BSS. */
     numplayers = 1;
     printf("[LOAD] nump=%d (from file), numplayers=%d\n", nump, numplayers);
     if(nump != numplayers)
     {
        kclose(fil);
		if(ud.recstat != 2)
...
int saveplayer(int8_t spot)
{
	...
	else
	{
     ...
     if ((fil = fopen(fullpathsavefilename,"wb")) == 0) {
         printf("[SAVE] fopen FAILED\n");
         return(-1);
     }
     printf("[SAVE] fopen OK, bv=%d, multimode=%d\n", bv, ud.multimode);
    ...
    for(i=0;i<MAXSPRITES;i++)
    {
     ...
     dfwrite(&parallaxyscale,sizeof(parallaxyscale),1,fil);
         ...
         fflush(fil);
         printf("[SAVE] fflush done, closing file\n");
         fclose(fil);
         printf("[SAVE] fclose done\n");
...
else
{
        ...
        case 300:
            ...
            {
                /* ESP32-P4: auto-fill empty save slot with default name */
                if(ud.savegame[current_menu-360][0] == 0)
                {
                    sprintf(ud.savegame[current_menu-360], "SAVE %d", current_menu-360);
                    inputloc = strlen(ud.savegame[current_menu-360]);
                }
                sprintf(text,"PLAYERS: %-2d                      ",ud.multimode);
                gametext(160,158,text,0,2+8+16);
                sprintf(text,"EPISODE: %-2d / LEVEL: %-2d / SKILL: %-2d",1+ud.volume_number,1+ud.level_number,ud.player_skill);
