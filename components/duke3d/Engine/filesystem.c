...
int32_t kread(int32_t handle, void *buffer, int32_t leng){
    ...
    if (!openFile->used){
        printf("Invalide handle in kread. Unrecoverable error.\n");
        return 0;
    }
...
int32_t klseek(int32_t handle, int32_t offset, int whence){
    ...
    if (!openFiles[handle].used){
        printf("Invalide handle in klseek.\n");
        return -1;
    }
...
int32_t kfilelength(int32_t handle)
{
    ...
    if (!openFile->used){
        printf("Invalide handle in kfilelength.\n");
        return 0;
    }
...
void kclose(int32_t handle)
{
    ...
    if (!openFile->used){
        printf("Invalide handle in kclose.\n");
        return;
    }
    if (openFile->type == SYSTEM_FILE){
        close(openFile->fd);
