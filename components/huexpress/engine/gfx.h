...
#define gfx_init() \
    ...
    gfx_need_redraw = 0; \
    IO_VDC_05_CR.W = 0; \
    if (pce_post_gfx_init_cb) { pce_post_gfx_init_cb(); pce_post_gfx_init_cb = NULL; }
...
extern uint16 pce_display_cr;  /* CR captured at display start, not during VBLANK */
extern void (*pce_post_gfx_init_cb)(void);  /* called once right after gfx_init() */
extern volatile int pce_resume_cr_protect;  /* frames to protect restored CR after resume */
extern uint16 pce_resume_cr_value;          /* the correct CR to use during protection */
