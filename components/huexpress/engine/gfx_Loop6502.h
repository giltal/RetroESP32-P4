    ...
    if (scanline < 14) {
        ...
        if (scanline == io.vdc_min_display) {
            gfx_need_redraw = 0;
            pce_display_cr = IO_VDC_05_CR.W;

            save_gfx_context(0);

            /* After resume, the game's VBLANK code may have zeroed VDC5
               before display start.  Override with the known-good value
               from the save state until the game naturally stabilises. */
            if (pce_resume_cr_protect > 0) {
                saved_gfx_context[0].cr = pce_resume_cr_value;
                IO_VDC_05_CR.W = pce_resume_cr_value;
                pce_display_cr = pce_resume_cr_value;
                pce_resume_cr_protect--;
            }
...
jump_CheckSprites:
    ...
    if ((return_value != INT_IRQ) && (return_value != INT_QUIT) && io.vdc_pendvsync) {
        io.vdc_status |= VDC_InVBlank;
        return_value = INT_IRQ;
        io.vdc_pendvsync = 0;
    }

    if (return_value == INT_QUIT) {
        I = INT_QUIT;
    } else if (return_value == INT_IRQ) {
        if (!(io.irq_mask & IRQ1)) {
            io.irq_status |= IRQ1;
            I = return_value;
        } else I = INT_NONE;
