...
{
  /* Z80 register pairs use a union trick: the 16-bit register value overlaps
     the low 16 bits of a byte* pointer.  The base address MUST be 64KB-aligned
     so that setting a 16-bit register value produces a valid pointer. */
  byte *mem = (byte *) heap_caps_aligned_alloc(0x10000, 0x10000, MALLOC_CAP_SPIRAM);
  if(mem == NULL) {
    fprintf(stderr, "Z80: Out of memory!\n");
    abort();
  }
  printf("Z80: allocated 64KB from PSRAM at %p (aligned)\n", mem);
  return mem;
}
