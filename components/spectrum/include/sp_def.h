...
extern unsigned char ay_read_data(void);
...
{                                                                \
    ...
    else if(((porth) & 0xC0) == 0xC0 && !((portl) & 0x02)) {    \
       dest = ay_read_data();                                    \
    }                                                            \
    else {                                                       \
       register int scri;                                        \
...
extern void ay_select_reg(unsigned char reg);
extern void ay_write_data(unsigned char val);
...
{                                                                \
  ...
  /* AY-3-8912 register select: A15=1, A14=1 */                  \
  if(((porth) & 0xC0) == 0xC0 && !((portl) & 0x02))             \
     ay_select_reg((source));                                    \
  /* AY-3-8912 data write: A15=1, A14=0 */                       \
  else if(((porth) & 0xC0) == 0x80 && !((portl) & 0x02))        \
     ay_write_data((source));                                    \
