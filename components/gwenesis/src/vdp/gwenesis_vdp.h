...
/* Rename VRAM to avoid collision with other emulator components (huexpress) */
#define VRAM gwenesis_VRAM

#define BIT_GEN(v, idx) (((v) >> (idx)) & 1)
#define BITS_GEN(v, idx, n) (((v) >> (idx)) & ((1 << (n)) - 1))

// VDP registers
#define REG0_DISABLE_DISPLAY (gwenesis_vdp_regs[0] & 1)
#define REG0_HVLATCH BIT_GEN(gwenesis_vdp_regs[0], 1)
#define REG0_LINE_INTERRUPT BIT_GEN(gwenesis_vdp_regs[0], 4)
#define REG1_PAL BIT_GEN(gwenesis_vdp_regs[1], 3)
#define REG1_240_LINE ((gwenesis_vdp_regs[1] & 0x08) >> 3)
#define REG1_DMA_ENABLED BIT_GEN(gwenesis_vdp_regs[1], 4)
#define REG1_VBLANK_INTERRUPT BIT_GEN(gwenesis_vdp_regs[1], 5)
#define REG1_DISP_ENABLED BIT_GEN(gwenesis_vdp_regs[1], 6)
#define REG2_NAMETABLE_A (BITS_GEN(gwenesis_vdp_regs[2], 3, 3) << 13)
#define REG3_NAMETABLE_W BITS_GEN(gwenesis_vdp_regs[3], 1, 5)
#define REG4_NAMETABLE_B (BITS_GEN(gwenesis_vdp_regs[4], 0, 3) << 13)
...
#define REG10_LINE_COUNTER BITS_GEN(gwenesis_vdp_regs[10], 0, 8)
#define REG10_COLUMN_COUNTER BITS_GEN(gwenesis_vdp_regs[10], 8, 15)
...
#define REG17_WINDOW_HPOS BITS_GEN(gwenesis_vdp_regs[17], 0, 5)
#define REG17_WINDOW_RIGHT ((gwenesis_vdp_regs[17] & 0x80) >> 7)
#define REG18_WINDOW_DOWN ((gwenesis_vdp_regs[0x12] & 0x80) >> 7)
#define REG18_WINDOW_VPOS BITS_GEN(gwenesis_vdp_regs[18], 0, 5)
...
#define REG23_DMA_TYPE BITS_GEN(gwenesis_vdp_regs[23], 6, 2)
...
#define CRAM_R(c)          COLOR_3B_TO_8B(BITS_GEN((c), 1, 3))
#define CRAM_G(c)          COLOR_3B_TO_8B(BITS_GEN((c), 5, 3))
#define CRAM_B(c)          COLOR_3B_TO_8B(BITS_GEN((c), 9, 3))

#define MODE_SHI           BITS_GEN(gwenesis_vdp_regs[12], 3, 1)
...
void gwenesis_vdp_set_buffer(unsigned char *ptr_screen_buffer);
