...
void load_sram()
{
    ...
    if (romName)
    {
        ...
        /* Load from: /sd/odroid/data/nes/<fileName>.sav */
        char pathBuf[512];
        snprintf(pathBuf, sizeof(pathBuf), "%s/odroid/data/nes/%s.sav", SD_BASE_PATH, fileName);
        printf("load_sram: loading from '%s'\n", pathBuf);
        state_load(pathBuf);
