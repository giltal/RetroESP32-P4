...
{
   /* Destroy NES machine if still alive (prevents memory leak across runs) */
   if (console.machine.nes)
   {
      /* rom and vrom point into the PSRAM rom_data buffer managed by
         nofrendo_run — null them so rom_free() won't try to free them */
      if (console.machine.nes->rominfo)
      {
         console.machine.nes->rominfo->rom = NULL;
         console.machine.nes->rominfo->vrom = NULL;
      }
   ...
   if (console.filename)
   {
      ...
      /* nextfilename may point to the same memory — avoid double-free */
      if (console.nextfilename == console.filename)
...
//   atexit(shutdown_everything);
   ...
   /* nofrendo_quit_flag is set by DoQuit() in video_audio.c when user presses MENU */
   extern volatile bool nofrendo_quit_flag;

   while (false == console.quit && false == nofrendo_quit_flag)
   {
