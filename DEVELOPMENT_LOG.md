...
### Phase 39: Atari 800 Save/Load State Fix

**Problem:** Save state appeared to work (showed "Saved!" in menu) but load never restored the game. Resuming from the launcher also failed silently.

**Root cause:** `LIBATARI800` is defined in `config.h`, which replaces `StateSav_SaveAtariState`/`StateSav_ReadAtariState` file I/O with **in-memory buffer** operations. The `mem_open()` replacement ignores the filename entirely and reads/writes to `LIBATARI800_StateSav_buffer`. The `mem_close()` replacement is a no-op. So:
- `SaveState()` called `StateSav_SaveAtariState(save_path, "wb", TRUE)` — serialized state into an in-memory buffer, **never wrote to the SD card file**
- `LoadState()` called `StateSav_ReadAtariState(save_path, "rb")` — read from the same (stale) in-memory buffer, **never read the SD card file**
- The save file on SD was never created, so `a800_check_save_exists()` always returned false after restart
- Within a single session, save then immediate load appeared to work (same buffer), masking the bug

**Fix:** Replaced `SaveState()`/`LoadState()` to use the proper LIBATARI800 API:
- Allocate a 210KB `emulator_state_t` buffer in PSRAM (lazy, freed on cleanup)
- **Save:** `LIBATARI800_StateSave(buffer, &tags)` serializes state into the buffer, then `fwrite()` flushes the buffer to `/sd/odroid/data/a800/<rom>.sav`
- **Load:** `fread()` reads the file into the buffer, then `LIBATARI800_StateLoad(buffer)` deserializes back into the emulator
- Buffer freed in cleanup section on emulator exit

**Fix part 2 — Launcher resume detection:**
The launcher's `has_save_file()` used `opendir()`/`readdir()` to scan the save directory, then truncated each filename by 4 chars and compared to `save_name`. This was fragile (leaked directory handles, broke on names with dots) and failed to find the `.sav` file. Replaced with a direct `stat()` check on the exact save file path.

**Fix part 3 — State buffer size mismatch:**
`atari800_run.cpp` defined `STATESAV_MAX_SIZE` as `210 * 1024` (215040) while `libatari800.h` and `statesav.h` defined it as `210000`. The emulator wrote 215040 bytes into a 210000-byte buffer — a 5040-byte overflow that corrupted state data. On resume, `StateSav_ReadAtariState` hit 5 "State file I/O failed" errors (reads past the buffer boundary returned 0), then the corrupted state caused a crash at address 0x00000000 (NULL function pointer in restored CPU state). Fixed by removing the conflicting local define and increasing the canonical `STATESAV_MAX_SIZE` to 300000 in both `libatari800.h` and `statesav.h`.

**Fix part 4 — BINLOAD re-load after state restore:**
For `.xex` files, `BINLOAD_Loader()` sets `BINLOAD_start_binloading=TRUE` and keeps the file handle open. After `LoadState()` restores state, the first emulation frame's SIO handler sees this flag and re-loads the `.xex`, overwriting the restored state. Fixed by clearing `BINLOAD_start_binloading`, `BINLOAD_loading_basic`, and closing `BINLOAD_bin_file` after successful `LoadState()`.

**Files changed:**

| File | Change |
|------|--------|
| `components/atari800/atari800_run.cpp` | Rewrote `SaveState()`/`LoadState()` with `LIBATARI800_StateSave`/`StateLoad` + SD file I/O; PSRAM buffer; BINLOAD clearing after resume; removed conflicting `STATESAV_MAX_SIZE` define |
| `components/atari800/libatari800.h` | `STATESAV_MAX_SIZE` 210000 → 300000 |
